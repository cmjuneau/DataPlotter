...
CMJ              July 12, 2017     Added 'c' flag for "minor" comments, changed
                                        default annotation location
CMJ              May 25, 2018     Began expansion for plotting other data types;
...
# rcParams['figure.figsize'] = 15.0, 6.5  # ANS Poster Sizes
...
mkrSize = 5  # 11 for ANS poster  # Size of markers in plot
mkrWidth = 1  # 2 for ANS poster  # Thickness or width of markers for data
# Fonts
myFont = 11  # 20 for ANS poster
matplotlib.rcParams.update({'font.size': myFont})  # Updates font for legend
myFont = 14  # 28 for ANS poster
...
class PlotClass:
    ...
    __plotAngle = -1  # Default plot angle
    __plotAngles = []  # Default plot angle
    ...
    __annotateX = 0.015  # Use set positioning for annotation (0<x<1)
    __annotateY = 0.1  # Use set positioning for annotation (0<y<1)
    __save = ""  # File not saved as default
    __xLabel = ""  # X Label
    __yLabel = ""  # Y Label
    __labelX = ""  # X Label (user defined)
    __labelY = ""  # Y Label (user defined)
    __reformat = 0  # Reformat data file if requested
    __showPlot = True # Show plot to user
    __xMin = 0 # Minimum x value for plot
    __xMax = 250 # Upper x axis limit
    __yMax = 0 # Default y axis limit (unused if 0)
    __yMin = 0 # Default y axis limit (unused if 0)
...
#    options for legend locations = {'best': 0, 'upper right': 1, 'upper left':
    ...
    __numLines = 0 # Number of lines to put on a single plot
    __scaleYAngles = 1 # used for scaling up/down based on many angles
    __scaleYAnglesEXP = 1 # Scaling of exp. data
    __scaleFactor = 2 # Used for separation of lines when multiple present
    __annotateIts = []
    __annotateXs = []
    __annotateYs = []
    __numAnnotations = 0 # Number of annotation to put on a single plot (should be same as __numLines)
    __xVal = 0.95
    __yVal = 0.0
    ...
    def setPlotAngle(self, plotAngle):
        ...
        self.__numLines = self.__numLines + 1 # Add a line to the graph
        self.__plotAngles.append(plotAngle)

    def getPlotAngle(self):
        # Only send back last angle
        return self.__plotAngles[self.__numLines-1]
    ...
    # --------------------------------------------------------------------------
    def setAnnotateIts(self, annotateIt):
        self.__numAnnotations = self.__numAnnotations + 1
        self.__annotateIts.append(annotateIt)

    # --------------------------------------------------------------------------
    def setAnnotateXs(self, annotateX):
        if ( annotateX <= 0 or annotateX >= xMax ):
            # Use defaults
            self.__annotateXs.append(self.__xVal * self.__xMax)
        else:
            self.__annotateXs.append(annotateX)

    # --------------------------------------------------------------------------
    def setAnnotateYs(self, annotateY):
        if ( annotateY <= 0 ):
            # Use defaults
            annotateY = 10**( - self.__scaleFactor * self.__numAnnotations)
            if ( self.__yMax > 0 ):
                # use yMax as reference
                annotateY = annotateY * self.__yMax
        self.__annotateYs.append(annotateY)
    ...
    # --------------------------------------------------------------------------
    def setLabelX(self, xLabel):
        self.__labelX = xLabel

    def getLabelX(self):
        return self.__labelX

    # --------------------------------------------------------------------------
    def setLabelY(self, yLabel):
        self.__labelY = yLabel
    ...
    def checkErrors(self):
        ...
        if self.__pID == 0:
            ...
            print("Warning Detected, PROGRAM Exiting...")
            return 1
        elif self.__numLines == 0:
            print('ERROR: Data type not specified for plot '
    ...
    # --------------------------------------------------------------------------
    def setPlotDetails(self):  # Setting title, axis labels, and annotations
        ...
        # Change label if user defined
        if not ( self.__labelX == '' ):
            plt.xlabel(self.__labelX, fontsize=myFont)
        if not ( self.__labelY == '' ):
            plt.ylabel(self.__labelY, fontsize=myFont)
        ...
        # Axis limits for X
        if self.__xMin < 0:
            print("ERROR: Minimum X-Value cannot be < 0. Setting to 0.")
            self.__xMin = 0
        elif self.__xMin >= self.__xMax:
            print("ERROR: Minimum X-Value cannot be greater than or equal to ", \
            self.__xMax, " (X-Maximum). Setting to 0.")
            self.__xMin = 0
        ...
        # Axis limits for Y
        if self.__yMax > 0 or self.__yMin > 0:
        ...
        if not self.__scalerX == 1 or not self.__scalerY == 1:
            ...
            print("\t Y-Scaling: %.1f" %(self.__scalerY))
            print("\t X-Scaling: %.1f" %(self.__scalerX))
    ...
    def createName(self):
        ...
        # Creating name
        if self.__plotType == 'dd' or self.__plotType == 'ei' or self.__plotType == 'ai':

            self.__plotName = temp + " Spectra (" + self.__pName
            # Adding Angle if appropriate
            if self.__plotType == 'dd':
                if self.__numLines == 1:
                    self.__plotName = self.__plotName + " at " + \
                    str(self.__plotAngle) + " degrees)"
                else:
                #    self.__plotName = self.__plotName + " at several angles)"
                    self.__plotName = self.__plotName + ")"
            else:
                self.__plotName = self.__plotName + ")"

        else:
            self.__plotName = '' # No plot name otherwise
    ...
    def getSimulatedData(self):
        ...
        if self.__plotType == 'dd' or self.__plotType == 'ai':
            ...
            # Set specific angle for plot
            for indx3 in range(0, self.__numLines, 1):
                # Create angle
                self.__plotAngle = self.__plotAngles[indx3]
                # Set scaling factor so plots don't overlap (reduce confusion)
                self.__scaleYAngles = 10**(-self.__scaleFactor*indx3)
                if not ( self.__scaleYAngles == 1 ):
                    print("NOTE: Scaling Y-values by ", self.__scaleYAngles, \
                    " for the ", self.__plotAngle, " degree data.")
                ...
                for indx2 in range(0, labelIndx + 1, 1):
                    ...
                    # Sorting through stored data at specific angles
                    # ----------------------------------------------------------
                        ...
                        if partID[indx1] == self.__pID:
                            ...
                            if angleDat[indx1] == self.__plotAngle:
                                ...
                                if not dataX[indx2][indx1] == 0 and not \
                                    ...
                                    yVal.append(self.__scaleYAngles*dataX[indx2][indx1])
                    ...
                    if plotSet:
                        ...
                        # Place text onto graph for multiple plots
                        if ( self.__numLines > 0 ):
                            myPlot.placeAnnotation(indx3)

                        temp2 = indx2 % len(labels)
                        temp3 = indx2 % len(linestyles)
                        temp4 = indx3 % len(linecolors)

                        if ( indx3 == 0 ):
                            # Include legend labels
                            plt.semilogy(xVal, yVal, label=labels[temp2],
                                         linestyle=linestyles[temp3],
                                         color = linecolors[temp4],
                                         alpha= opacity
                                        # linestyle=(linecolors[indx3]+linestyles[indx3]),#'None',
                                        # marker=markers[indx3+1],
                                        # mfc=markerFill[fillIndx],
                                        # markersize=tempSize,
                                        # markeredgewidth=mkrWidth
                                        )
                        else:
                            # Exclude legend labels
                            plt.semilogy(xVal, yVal,
                                         linestyle=linestyles[temp3],
                                         color = linecolors[temp4],
                                         alpha= opacity
                                    #     color = linecolors[temp4]
    ...
    def createExpData(self):
        ...
        dEDat = []
        CSDat = []
        # dCSDat = []  # EXFOR does not always have this for each data point
        ...
        for indx1 in range(0, len(labels) + 1, 1):
            ...
            for indx2 in range(0, len(expData[indx1]), 1):
                ...
                if labels[indx1] == 'E'.lower() or labels[indx1] == \
                        'energy'.lower():
                    # Store energy values
                    energyDat.append(expData[indx1][indx2])

                if labels[indx1] == 'dE'.lower():
                    # Store Cross Section Values
                    dEDat.append(expData[indx1][indx2])

                if labels[indx1] == 'angle'.lower() or labels[indx1] == \
                        'A'.lower():
                    # Store angle values
                    angleDat.append(expData[indx1][indx2])

                if labels[indx1] == 'dA'.lower():
                    # Store angle values
                    dADat.append(expData[indx1][indx2])

                if labels[indx1] == 'CS'.lower():
                    # Store Cross Section Values
                    CSDat.append(expData[indx1][indx2])

                # if labels[indx1] == 'dCS'.lower():
                    # Store Cross Section Values
                    # dCSDat.append(expData[indx1][indx2])
        ...
        for indx3 in range(0, self.__numLines, 1):
            ...
            self.__plotAngle = self.__plotAngles[indx3]
            # Set scaling factor so plots don't overlap (reduce confusion)
            self.__scaleYAnglesEXP = 10**(-self.__scaleFactor*indx3)

            if not ( self.__scaleYAnglesEXP == 1 ):
                # Scaling will be Used
                print("EXP. Data will be scaled by ", self.__scaleYAnglesEXP, \
                " for data at ", self.__plotAngle, " degrees.")
            ...
            if self.__plotType == 'dd'.lower():
                ...
                for indx1 in range(0, len(CSDat), 1):
                    if self.__plotAngle == angleDat[indx1]:
                        ...
                        yDat.append(CSDat[indx1] * self.__scaleYAnglesEXP)
                        # Error bars
                        if not len(dADat)==0:
                            xError.append(dADat[indx1])
                        # if not len(dCSDat)==0:
                        #     yError.append(dCSDat[indx1])
            ...
            if not self.__scaleX == 1:
                ...
                for indx in range(0, len(xDat), 1):
                    ...
                    if not len(xError) == 0:
                        # Scale X error if it exists
                        xError[indx] = xError[indx] * self.__scaleX
            ...
            if not self.__scaleY == 1:
                ...
                for indx in range(0, len(yDat), 1):
                    ...
                    # if not len(yError) == 0:
                    #     # Scale X error if it exists
                    #     yError[indx] = yError[indx] * self.__scaleY
            ...
            if ( len(yDat) > 0 ):
                plt.semilogy(xDat, yDat, label='Exp. Data',
                             ...
                             color='0.35',
                             markeredgewidth=mkrWidth+1)



    # --------------------------------------------------------------------------
    def placeAnnotation(self, indx):


        # Annotation for multiple lines
        if self.__numLines > 1:

                # Ensure within array limits
                if ( indx >= len(self.__annotateIts) ):
                    # ERROR
                    print("ERROR: Array limits exceeded for multiple annotations. Using modulus.")
                    indsx = indx % len(self.__annotateIts)


                if self.__numAnnotations == self.__numLines:
                    # Check if using angle or particle Name
                    if ( self.__annotateIts[indx].lower() == 'Particle'.lower() ):
                        # Use particle name instead
                        self.__annotateIts[indx] = self.__pName
                    elif ( self.__annotateIts[indx].lower() == 'Angle'.lower() ):
                        # Use angle according to  str(self.__plotAngles[indx3])
                        self.__annotateIts[indx] = str(self.__plotAngles[indx])

                    label = self.__annotateIts[indx]
                    xVal = self.__annotateXs[indx]
                    yVal = self.__annotateYs[indx]

                    # Check if xFrac > xMax
                    if ( xVal >= self.__xMax ):
                        xVal = xVal * self.__xMax


                    # Placing annotation on to graph if Desired
                    if not ( self.__annotateIts[indx].lower() == 'none' ):
                        makeAnnotation = True

                else:

                    # Create annotation for line within plot (defaults)
                    if ( self.__plotType == 'dd' ):
                        label = str(self.__plotAngles[indx])
                    else:
                        label = self.__pName

                    # Set X, Y annotation locations
                    xVal = self.__xVal * self.__xMax
                    yVal = 10**( - self.__scaleFactor * indx)
                    if (  self.__yMax > 0 ):
                        yVal = yVal * self.__yMax

                    makeAnnotation = True


                if ( makeAnnotation ):
                    temp = indx % len(linecolors)
    ...
    def annotateRxn(self):
        ...
        if not self.__annotateIt == '':
            ...
            plt.annotate(self.__annotateIt,
    ...
    if temp.strip().lower() == 'end plot':
        ...
        if flag == 'particle':
            ...
            if item == 'n'.lower():
                pID = 1
                myPlot.setPName('n')
            elif item == 'p'.lower() or item == 'H1'.lower():
                pID = 2
                myPlot.setPName('p')
            elif item == 'd'.lower() or item == 'H2'.lower():
                pID = 3
                myPlot.setPName('d')
            elif item == 't'.lower() or item == 'H3'.lower():
                pID = 4
                myPlot.setPName('t')
            elif item == 'He3'.lower():
                pID = 5
        ...
        # ---------------------------------------------------------------------
            ...
            # Obtain index of spaces (if any) in angle card for all spaces
            spaceIndx = 0
            while not ( spaceIndx == -1 ):
                spaceIndx = item.strip().find(' ', 0, len(item))
                if ( spaceIndx == -1 ):
                    # No new spaces, don't use spaceIndx for angles
                    angle = item
                else:
                    angle = item[:spaceIndx+1].lower()  # Obtain angle
                    item = item[spaceIndx + 1:].strip() # Remove angle from string
        ...
        # ---------------------------------------------------------------------
            ...
            # ---------------------------------------------------------------------
        elif flag == 'annotateLine'.lower():

            # Annotation exists, input in format: (Label), X, Y

            # Get Label
            spaceIndx = item.find(' ', 0, len(item))
            label = item[:spaceIndx].lower().strip()  # Item NOT case sensitive
            item = item[spaceIndx + 1:].strip()
            if ( spaceIndx == -1 ):
                print("   ERROR: Missing x location for line annotation. Setting to default (", 100*myPlot.getXVal(), "%\ of xMax).")
                xVal = 0
            ...
            if not ( spaceIndx == -1 ):
                spaceIndx = item.find(' ', 0, len(item))
                if ( spaceIndx == -1 ):
                    print("   ERROR: Missing y location for line annotation. Setting to default.")
                    xVal = float( item.lower().strip() )
                    yVal = 0
                else:
                    xVal = float( item[:spaceIndx].lower().strip() )
                    item = item[spaceIndx + 1:].strip()
            ...
            if not ( spaceIndx == -1 ):
                spaceIndx = item.find(' ', 0, len(item))
                if ( spaceIndx == -1 ):
                    yVal = float( item.lower().strip() )
                else:
                    yVal = float( item[:spaceIndx].lower().strip() )
                    item = item[spaceIndx + 1:].strip()


            myPlot.setAnnotateIts(label)
            myPlot.setAnnotateXs(xVal)
            myPlot.setAnnotateYs(yVal)

            if ( testing > 1 ):
                print("line label, x/y = ", label, ", ", xFrac, "/", yFrac)
        ...
        # ---------------------------------------------------------------------
            ...
            # General Comment for Input file, do nothing
            if testing > 1:
                print('Comment Detected, ignoring input line.')
        ...
        # ---------------------------------------------------------------------
        elif flag == 'scaleFactor'.lower():

            # Changing to integer
            scaleFactor = int(item.strip().lower())
            myPlot.setScaleFactor(scaleFactor)

            # For Testing
            if testing > 1:
                print('Scale factor changed to %d.' %(myPlot.getScaleFactor()))

        # ---------------------------------------------------------------------
        elif flag == 'xMin'.lower():

            # Changing to integer
            xMin = int(item.strip().lower())
            myPlot.setXMin(xMin)

            # For Testing
            if testing > 1:
                print('x Minimum changed to %d.' %(myPlot.getXMin()))
        ...
        # ---------------------------------------------------------------------
        elif flag == 'yMax'.lower():

            # Changing to integer
            yMax = float(item.strip().lower())
            myPlot.setYMax(yMax)

            # For Testing
            if testing > 1:
                print('y Maximum changed to %d.' %(myPlot.getYMax()))

        # ---------------------------------------------------------------------
        elif flag == 'yMin'.lower():

            # Changing to integer
            yMin = float(item.strip().lower())
            myPlot.setYMin(yMin)

            # For Testing
            if testing > 1:
                print('y Minimum changed to %d.' %(myPlot.getYMin()))
        ...
        # ---------------------------------------------------------------------
        elif flag == 'xLabel'.lower():

            # Assigning answer to plot
            if not item == '':
                myPlot.setLabelX(item)
            else:
                # Error - invalid item
                print("ERROR: Input error for plot %d, line %d: Invalid "
                      "X label Item - \"%s\"." % (plotNum, linesUsed, item))
                print("Warning Detected, PROGRAM Exiting...")
                break

            # For Testing
            if testing > 1:
                print('%s (%s) became %s.' % (flag, item, myPlot.getLabelX()))

        # ---------------------------------------------------------------------
        elif flag == 'yLabel'.lower():

            # Assigning answer to plot
            if not item == '':
                myPlot.setLabelY(item)
            else:
                # Error - invalid item
                print("ERROR: Input error for plot %d, line %d: Invalid "
                      "Y label Item - \"%s\"." % (plotNum, linesUsed, item))
                print("Warning Detected, PROGRAM Exiting...")
                break

            # For Testing
            if testing > 1:
                print('%s (%s) became %s.' % (flag, item, myPlot.getLabelY()))
