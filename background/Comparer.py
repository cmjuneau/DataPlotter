...
def parseTime(src):
  ...
  hourT = 0
  if ( "hour" in text ):
    hourIndx = text.find("hour")
    hourT = float( text[ : hourIndx ].strip() ) # Obtain number of hours
    text = text[ hourIndx + len("hour,") + 1 : ].strip()   # Remove "XX.YY hour," from text
...
def compare(nameA, nameB, outName):
    ...
    if ( numDifferences > maxAllowedDiffs ):
        theSummary = "W:   There are %d differences between %15s and %15s." % ( numDifferences, nameA, nameB )
    else:
